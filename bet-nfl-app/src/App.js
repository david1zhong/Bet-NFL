...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex">
      <div className={`bg-[#1c1e1d] p-4 min-h-screen ${isMobile ? 'w-full pb-96' : 'flex-1'}`} 
           style={{ width: isMobile ? '100%' : 'max(60%, 535px)' }}>
      ...
      <div className={`bg-[#0a0a0a] p-4 ${ 
        isMobile ? 'fixed bottom-0 left-0 right-0 h-96 overflow-y-auto border-t-2 border-gray-600' : 'flex-1 min-w-80'
      }`}>
        ...
        {selectedBets.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64">
            <svg xmlns="http://www.w3.org/2000/svg" width="581" height="457" viewBox="0 0 581 457" preserveAspectRatio="xMidYMid meet">
              <image href="=" width="495" height="423"/>
            </svg>
            <p className="text-gray-400 text-center mt-4">
              Select bets from the games above to build your betslip
            </p>
          </div>
        ) : (
          <>
